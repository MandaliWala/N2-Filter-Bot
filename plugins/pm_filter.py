...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        search = message.text
        temp_files, temp_offset, total_results = await get_custom_search_results(query=search.lower(), offset=0, filter=True)
        if total_results == 0:
            return
        else:
...
async def next_page(bot, query):
    ...
    files, n_offset, total = await get_custom_search_results(search, offset=offset, filter=True)
    try:
...
async def advantage_spoll_choker(bot, query):
    ...
    files, offset, total_results = await get_custom_search_results(movie, offset=0, filter=True)
    if files:
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    files, offset, total_results = await get_custom_search_results(search, offset=0, filter=True)
    if not files:
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    files, _, _ = await get_custom_search_results(search, max_results=10)
    files = [file for file in files if re.search(seas, file.file_name, re.IGNORECASE)]
    
    seas1 = "s01" if seas == "season 1" else "s02" if seas == "season 2" else "s03" if seas == "season 3" else "s04" if seas == "season 4" else "s05" if seas == "season 5" else "s06" if seas == "season 6" else "s07" if seas == "season 7" else "s08" if seas == "season 8" else "s09" if seas == "season 9" else "s10" if seas == "season 10" else ""
    search1 = f"{search1} {seas1}"
    BUTTONS1[key] = search1
    files1, _, _ = await get_custom_search_results(search1, max_results=10)
    ...
    files2, _, _ = await get_custom_search_results(search2, max_results=10)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if not search:
            await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name),show_alert=True)
            return
        files, n_offset, total = await get_custom_search_results(search, offset=int(offset), filter=True)
